...
export default function Form(){
    ...
    // 
    const firstDay = useMemo(() => formatDate(selectDate?.from), [selectDate?.from])
    const lastDay  = useMemo(() => formatDate(selectDate?.to),   [selectDate?.to])
            ...
            >
                ...
                <div className="lg:border w-full border-zinc-400 rounded-lg flex justify-start items-stretch flex-col lg:flex-row space-y-4 lg:space-y-0">
                    ...
                    <div ref={calendarRef} tabIndex={0} className={`${borderStyle}`}>
                        <div onClick={handleToggleCalendar} className={`flex items-center h-13 lg:w-65 w-full  space-x-5 cursor-pointer border border-zinc-300 rounded-md lg:border-0 ${borderStyle} `} >
